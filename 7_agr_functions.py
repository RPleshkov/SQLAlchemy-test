...
director1 = Director(name="Christopher", surname="Nolan", country="England", rating=100)
...
director5 = Director(name="David", surname="Fincher", country="USA", rating=99)
...
async def main():
    ...
    async def db_tests(session: AsyncSession):
        # stmt = select(func.round(func.avg(Director.rating), 4)).where(
        #     Director.country == "USA"
        # )
        # result = await session.execute(stmt)
        # print(*result.all())

        # stmt = select(func.count("*")).where(is_not(Director.rating, None))
        # result = await session.execute(stmt)
        # print(*result.all())

        # stmt = select(func.count(distinct(Director.country)))
        # result = await session.execute(stmt)
        # print(result.all())

        # stmt = select(func.min(Director.rating), func.max(Director.rating)).where(
        #     Director.country == "USA"
        # )
        # result = await session.execute(stmt)
        # print(result.all())

        stmt = select(func.string_agg(distinct(Director.country), ", "))
        result = await session.execute(stmt)
        print(result.all())
